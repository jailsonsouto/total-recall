...
class RecallContext:
    ...
    def to_dict(self) -> dict:
        """Serializa para dict (para --format json)."""
        return {
            "query": self.query,
    ...
    def to_markdown(self) -> str:
        """Exporta para Markdown (para --clip)."""
        lines = [
            f"# Recall: {self.query}",
            f"",
            f"*{len(self.results)} resultados de {self.sessions_searched} sessões*",
            f"",
        ]
        for i, r in enumerate(self.results, 1):
            ts = r.timestamp.strftime("%d/%m/%Y %H:%M") if r.timestamp else "?"
            sources = " + ".join(s.upper() for s in r.sources) if r.sources else "?"
            lines.append(f"## {i}. {r.project_label or r.session_id[:8]} — Sessão {r.session_id[:8]}")
            lines.append(f"*{ts} | score: {r.score:.3f} | {sources}*")
            lines.append("")
