...
class SQLiteVectorStore:
    ...
    def _fuzzy_find_variants(self, token: str, conn) -> list[str]:
        ...
        Se o token original não existe no vocabulário (doc_count=0),
        exige score ≥ 90% para evitar expansões irrelevantes.
        """
        try:
        ...
        token_doc_count = vocab.get(token, 0)

        # Se o token não existe no vocab, exige match muito próximo
        # para evitar expansões como sqlite → site, slice, split
        if token_doc_count == 0:
            score_cutoff = 90
            limit = FUZZY_MAX_EXPANSIONS * 2
        else:
        ...
        # Filtra self-match e termos que são claramente não-relacionados
        # Regra: se o candidato tem doc_count muito maior que o original,
        # provavelmente é um termo comum diferente, não um typo
        max_allowed = max(token_doc_count * 3, 8)
        candidates = [
            (m[0], m[1], vocab.get(m[0], 0))
            for m in matches
            if m[0] != token
            and vocab.get(m[0], 0) <= max_allowed
            and m[1] >= 80  # mínimo 80% de similaridade
        ]
    ...
    def _build_fts_query(self, query: str, conn) -> tuple[str, list[dict]]:
        ...
        for word in words:
            ...
            # Threshold relativo: expande se doc_count está no percentil mais baixo
            # do vocabulário (< 0.5% dos docs). Evita expandir termos que já são comuns.
            # Se o token já existe com doc_count razoável (>5), não expande.
            if len(clean) >= FUZZY_MIN_TOKEN_LENGTH:
                vocab = self._get_fts_vocabulary(conn)
                doc_count = vocab.get(clean, 0)
                total_docs = max(len(vocab), 1)
                # Threshold relativo: 0.5% do vocabulário total, mínimo 3
                relative_threshold = max(int(total_docs * 0.005), 3)
                # Se já existe com doc_count > 5 E > threshold, não expande
                if doc_count == 0 or (doc_count <= relative_threshold and doc_count <= 5):
    ...
    def hybrid_search(self, query: str, n_results: int = 5,
        ...
        # Graph Lite boost: chunks que compartilham entidades com os termos da query
        # recebem um pequeno boost proporcional à co-ocorrência
        ranked = self._apply_graph_boost(ranked, query)
    ...
    def _apply_graph_boost(self, ranked: list[dict], query: str) -> list[dict]:
        """Aplica boost baseado em co-ocorrência de entidades (Graph Lite).

        Se os termos da query matcham entidades que co-ocorrem em chunks,
        esses chunks recebem um boost proporcional.
        """
        try:
            with self.db.connection() as conn:
                # Encontra entidades que matcham termos da query
                query_terms = [w.strip(".,!?;:") for w in query.lower().split() if len(w.strip(".,!?;:")) >= 2]
                if not query_terms:
                    return ranked

                placeholders = ",".join("?" for _ in query_terms)
                matching_entities = conn.execute(
                    f"SELECT id, name FROM entities WHERE LOWER(name) IN ({placeholders})",
                ...
                entity_ids = [e[0] for e in matching_entities]
                id_placeholders = ",".join("?" for _ in entity_ids)

                # Encontra chunks que compartilham entidades com os termos da query
                boosted_chunk_ids = set()
                for eid in entity_ids:
                    # Co-ocorrência: chunks que têm esta E outra entidade da query
                    co_entities = conn.execute(
                        f"""SELECT DISTINCT ce2.chunk_id
                           FROM chunk_entities ce1
                           JOIN chunk_entities ce2 ON ce1.entity_id != ce2.entity_id
                               AND ce1.chunk_id = ce2.chunk_id
                           WHERE ce1.entity_id = ?""",
                        [eid],
                    ).fetchall()
                    for row in co_entities:
                        boosted_chunk_ids.add(row[0])

                if not boosted_chunk_ids:
                    return ranked

                # Aplica boost: +5% por entidade compartilhada
                for item in ranked:
            ...
            pass  # Graph boost é opcional — falha silenciosa
